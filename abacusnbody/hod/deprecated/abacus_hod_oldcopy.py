...
from desilike.theories.galaxy_clustering.base import APEffect
...
class AbacusHOD:
    ...
    def __init__(
        ...
        self.sim_ = sim_params.get('halo_lc', 0)
        ...
        # H.Z. for AP
        self.sim_fid = sim_params.get('sim_fid', 0)
        self.sim_cosmo = sim_params.get('sim_cosmo', 0)
        APobj=APEffect(cosmo=AbacusSummit(self.sim_cosmo,engine='camb'),
                          fiducial=AbacusSummit(self.sim_fid,engine='camb'),
                          z=self.z_mock,
                          mode='geometry')
        APobj.calculate()
        self.qper = np.float32(APobj.qper)
        self.qpar = np.float32(APobj.qpar)
        ...
        if not skip_staging:
            ...
            self.lbox_ap = np.float32([meta['BoxSize']/self.qper,meta['BoxSize']/self.qper,meta['BoxSize']/self.qpar])
    ...
    def staging(self):
        ...
        self.lbox_ap = np.float32([self.lbox/self.qper,self.lbox/self.qper,self.lbox/self.qpar])
    ...
    def compute_xirppi(self, mock_dict, rpbins, pimax, pi_bin_size, Nthread=8):
        ...
        for i1, tr1 in enumerate(mock_dict.keys()):
            x1 = mock_dict[tr1]['x']/self.qper
            y1 = mock_dict[tr1]['y']/self.qper
            z1 = mock_dict[tr1]['z']/self.qpar
            for i2, tr2 in enumerate(mock_dict.keys()):
                if i1 > i2:
                    continue  # cross-correlations are symmetric
                if i1 == i2:  # auto corr
                    clustering[tr1 + '_' + tr2] = calc_xirppi_fast(
                        x1, y1, z1, rpbins, pimax, pi_bin_size, self.lbox_ap, Nthread
                    )
                else:
                    x2 = mock_dict[tr2]['x']/self.qper
                    y2 = mock_dict[tr2]['y']/self.qper
                    z2 = mock_dict[tr2]['z']/self.qpar
                    clustering[tr1 + '_' + tr2] = calc_xirppi_fast(
                        x1,
                        y1,
                        z1,
        ...
        for i1, tr1 in enumerate(mock_dict.keys()):
            ...
            for i2, tr2 in enumerate(mock_dict.keys()):
                ...
                if i1 == i2:  # auto corr
                    ...
                    new_wp = calc_wp_fast(x1, y1, z1, rpbins, pimax, self.lbox_ap, Nthread)
            ...
            for i2, tr2 in enumerate(mock_dict.keys()):
                ...
                if i1 == i2:
                    ...
                    clustering[tr1 + '_' + tr2] = calc_wp_fast(
                        x1, y1, z1, rpbins, pimax, self.lbox_ap, Nthread
                    )
                else:
                    print(tr1 + '_' + tr2)
                    x2 = mock_dict[tr2]['x']/self.qper
